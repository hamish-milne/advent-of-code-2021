...
for c in open('day23.txt').read():
    ...
    if i in range(4):
        s.append(i)
StartState: State = ((s[4], 3, 3, s[0]), (s[5], 1, 2, s[1]), (s[6], 0, 1, s[2]), (s[7], 2, 0, s[3]), (None,)*7)
L = 4
        ...
        for (start, stop, step) in (left, right):
            for hi in range(start, stop, step):
                ...
                if hall[hi] is not None:
                    if hall[hi] == ri and all(x == ri for x in room):
                        yield (hi, TO_ROOM, ri, (moves-1) * (10 ** hall[hi])) # type: ignore
                    break
                elif len(room) > 0 and any(x != ri for x in room):
